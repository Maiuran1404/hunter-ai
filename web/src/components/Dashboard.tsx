...
import type { Opportunity, SentEmail, Subscription, CompanyProfile, EmailDraft } from '@/lib/types';
...
const CATEGORY_COLORS: Record<string, string> = {
  infrastructure: '#3b82f6', productivity: '#8b5cf6',
  design: '#ec4899', analytics: '#f59e0b', other: '#6b7280',
};
...
export default function Dashboard() {
  ...
  const [local, setLocal] = useState<LocalState>({
    loading: false, initialLoading: true, applyingId: null, showAllOpps: false,
  ...
  const emails = data?.sent_emails ?? [];
  const subs = data?.subscriptions ?? [];
  const profile = data?.profile ?? null;
  const totalValue = data?.total_potential_value ?? 0;
  const gmailConnected = data?.gmail_connected ?? false;
  const gmailAuthUrl = data?.gmail_auth_url ?? null;
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      const d = await api.fetchState();
      setData(d);
    } catch { /* ignore */ }
    setLocal(l => ({ ...l, loading: false, initialLoading: false }));
  }, []);
  ...
  const handleStartOver = useCallback(async () => {
    ...
    setData(null);
    setLocal({
      loading: false, initialLoading: false, applyingId: null, showAllOpps: false,
      draftEmail: null, sendingEmail: false, gmailPolling: false,
      activeView: 'welcome', websiteUrl: '', scanning: false, scanError: null,
      checkingReplies: false,
    });
  ...
  const handleApplyOne = useCallback(async (opp: Opportunity) => {
    ...
    try {
      const draft = await api.draftEmail(opp.id);
      if (draft.error) {
  ...
  const handleSendDraft = useCallback(async () => {
    ...
    try {
      ...
      if (result.email) {
        setData(d => d ? { ...d, sent_emails: [...d.sent_emails, result.email] } : d);
      }
      ...
      if (msg.toLowerCase().includes('gmail not connected')) {
        setData(d => d ? { ...d, gmail_connected: false } : d);
      }
    }
  }, [local.draftEmail, gmailConnected]);
  ...
  const handleScanWebsite = useCallback(async () => {
    ...
    try {
      const scanResult = await api.scanWebsite(url);
      if (scanResult.error) {
        setLocal(l => ({ ...l, scanning: false, scanError: scanResult.error }));
        return;
      }
      await api.findOpportunities();
      await refreshData();
  ...
  const handleFileUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLocal(l => ({ ...l, scanning: true, scanError: null }));
    try {
      const result = await api.analyzeStatement(file);
      if (result.error) {
        setLocal(l => ({ ...l, scanning: false, scanError: result.error }));
        return;
      }
      await api.findOpportunities();
      await refreshData();
  ...
  const handleCheckReplies = useCallback(async () => {
    setLocal(l => ({ ...l, checkingReplies: true }));
    try {
      await api.checkReplies();
      await refreshData();
    } catch { /* ignore */ }
    setLocal(l => ({ ...l, checkingReplies: false }));
  }, [refreshData]);

  const handleSendReply = useCallback(async (emailId: string) => {
    try {
      await api.sendReply(emailId);
      await refreshData();
    } catch { /* ignore */ }
  }, [refreshData]);
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        if (status.connected) {
          clearInterval(interval);
          setLocal(l => ({ ...l, gmailPolling: false }));
          setData(d => d ? { ...d, gmail_connected: true, gmail_auth_url: null } : d);
        }
      } catch { /* ignore */ }
    }, 3000);
  ...
  /* ═══ INITIAL LOADING ═══ */
  if (local.initialLoading) return (
  ...
  const repliedEmails = emails.filter(e => e.status === 'replied' && e.reply);
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        {subs.length > 0 && (
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              ...
              {opps.length === 0 && (
                <button
                  onClick={async () => { setLocal(l => ({ ...l, loading: true })); try { await api.findOpportunities(); await refreshData(); } catch {} setLocal(l => ({ ...l, loading: false })); }}
        ...
        {repliedEmails.length > 0 && (
          <div style={{ marginTop: 16 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: C.tertiary, textTransform: 'uppercase', letterSpacing: '0.06em', padding: '6px 2px', marginBottom: 6 }}>
              Replies ({repliedEmails.length})
            </div>
            {repliedEmails.map(email => (
              <div key={email.id} style={{ ...rowCardStyle, padding: '10px 14px', marginBottom: 4, flexDirection: 'column', alignItems: 'stretch', gap: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: C.primary }}>{email.reply!.from}</span>
                  {email.reply!.ai_response_status === 'drafted' && (
                    <button onClick={() => handleSendReply(email.id)}
                      style={{ padding: '4px 10px', border: 'none', borderRadius: 6, background: C.accent, color: '#fff', fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>
                      Send reply
                    </button>
                  )}
                  {email.reply!.ai_response_status === 'sent' && <StatusDot active={true} />}
                </div>
                <div style={{ fontSize: 12, color: C.secondary, lineHeight: 1.5, maxHeight: 60, overflow: 'hidden' }}>
                  {email.reply!.body.slice(0, 150)}...
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Check replies + Gmail status */}
        {emails.length > 0 && (
          <div style={{ marginTop: 16, display: 'flex', gap: 8 }}>
            <button onClick={handleCheckReplies} disabled={local.checkingReplies || !gmailConnected}
              style={{
                flex: 1, padding: '10px 16px', border: `1px solid ${C.btnSecBor}`, borderRadius: 8,
                background: C.btnSecBg, color: C.btnSecTx, fontSize: 13, fontWeight: 500,
                cursor: 'pointer', opacity: local.checkingReplies ? 0.5 : 1,
              }}>
              {local.checkingReplies ? 'Checking...' : 'Check replies'}
            </button>
            {!gmailConnected && (
              <a href={gmailAuthUrl || '#'} target="_blank" rel="noopener noreferrer" onClick={handleConnectGmail}
                style={{
                  padding: '10px 16px', borderRadius: 8,
                  background: C.btnPrimary, color: C.btnPriTx, fontSize: 13, fontWeight: 600, textDecoration: 'none',
